...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const paletteInput = [color0, color1, color2].filter(Boolean)
    const paletteColors = paletteInput.length > 0 ? paletteInput : colors

    function makePaletteTexture(stops) {
      const arr = Array.isArray(stops) && stops.length > 1 ? stops : [stops?.[0] || '#ffffff', stops?.[0] || '#ffffff']
      const w = arr.length
    ...
    const paletteTex = makePaletteTexture(paletteColors)
    ...
    const viscous_frag = `
      ...
      void main(){
        ...
        vec2 newv = (4.0 * old + v * dt * (new0 + new1 + new2 + new3)) / (4.0 * (1.0 + v * dt));
    ...
    class Advection extends ShaderPass {
      constructor(simProps) {
        ...
        this.uniforms = this.props.material.uniforms
        this.init()
        this.createBoundary(simProps.cellScale)
      }
      createBoundary(cellScale) {
      ...
      update({ dtValue, isBounce, BFECCValue }) {
        this.uniforms.dt.value = dtValue
        this.uniforms.isBFECC.value = BFECCValue
    ...
    class Viscous extends ShaderPass {
      ...
      update({ viscousValue, iterations, dtValue }) {
        let fboOut = this.props.output1
        this.uniforms.v.value = viscousValue
        this.uniforms.dt.value = dtValue
        for (let i = 0; i < iterations; i += 1) {
          const fboIn = i % 2 === 0 ? this.props.output0 : this.props.output1
    ...
    class Poisson extends ShaderPass {
      ...
      update({ iterations }) {
        ...
        for (let i = 0; i < iterations; i += 1) {
          const pIn = i % 2 === 0 ? this.props.output0 : this.props.output1
    ...
    class Simulation {
      ...
      update() {
        ...
        this.advection.update({
          dtValue: this.options.dt,
          isBounce: this.options.isBounce,
          BFECCValue: this.options.BFECC,
        })
        ...
        if (this.options.isViscous) {
          vel = this.viscous.update({ viscousValue: this.options.viscous, iterations: this.options.iterations_viscous, dtValue: this.options.dt })
        }
